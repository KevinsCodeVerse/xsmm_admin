      ...
      pickerOptions: {
        ...
        shortcuts: [
          ...
          },
          {
            ...
            onClick(picker) {
              const date = new Date();
              var thisWeekDay = moment().format("E");
              date.setTime(date.getTime() - 3600 * 1000 * 24 *(thisWeekDay-1+7));
          ...
          this.pickerOptions.shortcuts.push(
            ...
            },
            {
              ...
              onClick(picker) {
                picker.$emit(
                  "pick",
                  new Date(new Date().getFullYear() - 1 + "-01-01 00:00")
                );
              },
            }
          );
    ...
    getDate() {
      ...
      switch (this.date3) {
        case "week":
          ...
          // 获取本周的第一天
          var weekOneDay = this.$moment()
            .subtract(thisWeekDay - 1, "days") //修改过
            .format("Y-MM-DD");
          // 检查选择的日期是否在两个时间内
          if (
            this.$moment(selectDate).isBetween(weekOneDay, new Date()) ||
            selectDate == weekOneDay
          ) {
            // 返回周一到当前今天的时间
            var weeks = [];
            ...
            startTime = this.$moment(this.deftime)
              .subtract(0, "days")
              .format("YYYY-MM-DD");
            dateStrs = this.week;
          }
          endTime = this.$moment(startTime)
            .add(dateStrs.length - 1, "days")
            .format("Y-MM-DD");
          break;
        case "month":
          var selectDate = this.$moment(this.deftime).format("YYYY-MM-DD");
          ...
          if (
            thisMonth == this.deftime.getMonth() &&
            thisYear == this.deftime.getFullYear()
          ) {
            // 如果是本月
            this.month.splice(0);
        ...
        case "year":
          ...
          var days =
            this.deftime.getFullYear() == thisYear
              ? new Date().getDate()
              : "31";
          endTime = `${this.$moment(this.deftime).format("Y")}-${
            dateStrs.length
          }-${days}`;
    ...
    dateRangeChange(val) {
      if (val) {
        ...
        this.$emit("changeDate", {
          type: "day",
          dates: [
            this.$moment(val[0]).format("Y-MM-DD"),
            this.$moment(val[1]).format("Y-MM-DD"),
          ],
          dateStrs,
        });
      } else {
