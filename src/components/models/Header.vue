    ...
    <div class="flex-box header_top">
      <div style="display: flex; align-items: center;" :style="getWidth()">
        <div class="flexible_model">
          <i
            class="el-icon-s-fold flexible"
            v-show="$store.state.navOpen"
            @click="$store.state.navOpen = !$store.state.navOpen"
          ></i>
          <i
            class="el-icon-s-unfold flexible"
            v-show="!$store.state.navOpen"
            @click="$store.state.navOpen = !$store.state.navOpen"
          ></i>
        </div>
      ...
      <div class="header_right">
        ...
        <div style="margin: 0 10px;color: #666;">{{ roleName }}</div>
        <div>{{ name }}</div>
      </div>
    </div>
      ...
      <el-dropdown trigger="click" @command="closeTab" v-if="tabs.length > 1">
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item command="left"
            ><i class="el-icon-back"></i>关闭左侧</el-dropdown-item
          >
          <el-dropdown-item command="right"
            ><i class="el-icon-right"></i>关闭右侧</el-dropdown-item
          >
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    ...
    getWidth() {
      ...
      if (this.$store.state.isPhone) {
        ...
        widthStr = this.$store.state.navOpen
          ? "width:calc(100vw - 200px);margin-left:200px;"
